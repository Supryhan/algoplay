...
object Solution2 {
  def main(args: Array[String]) {
    val sc = new java.util.Scanner (System.in);
    val a0 = sc.nextInt();    val a1 = sc.nextInt();    val a2 = sc.nextInt()
    val b0 = sc.nextInt();    val b1 = sc.nextInt();    val b2 = sc.nextInt()
...
object Solution3 extends App {
  ...
  def m(): Unit = {
    ...
    if(t.isDefined && t.get.left.isDefined) {queue :+ t.get.left; left = true}
    if(t.isDefined && t.get.right.isDefined) {queue :+ t.get.right; right = true}
    if(queue.nonEmpty) m()
